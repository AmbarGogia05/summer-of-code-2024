...
def process_transaction(item_list_sorted, c_ID, s_ID, t_ID=None):
    try:
        ...
        transactionlog = TransactionHistory(
            ...
            s_id=transaction_object.s_id,
            date=transaction_object.date,
            time=transaction_object.time,
...
def create_transaction():
    verify_jwt_in_request(locations=['cookies'])  # Verify JWT in the cookie
...
def delete_transaction():
    ...
    if role != 'staff':
        flash('Not authorized to delete transactions!')
        return redirect(url_for('customer.login'))
    if role == 'staff' and not isadmin:
        flash('Not authorized to delete transactions!')
        return redirect(url_for('staff.staff_home'))
...
def find_transaction():
    ...
    if role != 'staff':
        flash('Not authorized to view transactions!')
        return redirect(url_for('customer.login'))
...
def update_transaction():
    ...
    if role != 'staff':
        flash('Not authorized to update transactions!')
        return redirect(url_for('customer.login'))
    if role == 'staff' and not isadmin:
        flash('Not authorized to update transactions!')
        return redirect(url_for('staff.staff_home'))
